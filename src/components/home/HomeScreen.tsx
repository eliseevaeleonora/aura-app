...
export default function HomeScreen() {
  ...
  const toggleHabit = (id: string) => {
    setHabits(prev => prev.map(h => {
      if (h.id !== id) return h
      if (!h.done) { addXP(h.xp); showXPToast(h.xp) }
    ...
    <div className="h-full overflow-y-auto scroll-hide">
      <motion.div variants={stagger.container} initial="initial" animate="animate" className="pb-6">
        ...
        <motion.div variants={stagger.item} className="px-5 pt-12 pb-5">
          <div className="flex items-start justify-between">
            ...
            <div className="w-11 h-11 rounded-full flex items-center justify-center text-xl flex-shrink-0"
              style={{ background: 'linear-gradient(135deg, var(--color-purple3), var(--color-pink-dim))', border: '1.5px solid var(--color-border2)' }}>
              ✦
            </div>
          </div>
        </motion.div>
        ...
        <motion.div variants={stagger.item}>
          <div className="flex gap-2 px-5 pb-5 overflow-x-auto scroll-hide">
            {[
              ...
              <div key={label} className="flex-shrink-0 flex items-center gap-2 px-3.5 py-2 rounded-full"
        ...
        <motion.div variants={stagger.item} className="px-5 pb-4">
          <Card gradient className="p-4">
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-display text-base font-normal" style={{ color: 'var(--color-lavender2)' }}>
                Путь к уровню {profile.level + 1}
              </h3>
              <span className="text-xs font-semibold px-2.5 py-1 rounded-lg"
                style={{ background: 'var(--color-purple-dim)', color: 'var(--color-purple2)', border: '1px solid var(--color-border2)' }}>
            ...
            {weekDays.map((day, i) => (
              <div key={day.date} className="flex flex-col items-center gap-1.5 flex-1">
                ...
                <motion.div
                  ...
                  style={day.isToday
                    ? { background: 'linear-gradient(135deg, var(--color-purple3), var(--color-pink-dim))', color: 'white', boxShadow: '0 0 12px rgba(155,138,232,0.4)' }
                    : day.completed
                    ? { background: 'var(--color-purple-dim)', border: '1px solid var(--color-purple)', color: 'var(--color-purple2)' }
                    : { background: 'var(--color-bg4)', border: '1px solid var(--color-border)', color: 'var(--color-text4)' }
                  }
                >
        ...
        <motion.div variants={stagger.item} className="px-5 pb-2">
          <SectionTitle right={`${completedCount}/${totalCount} выполнено`}>Сегодняшние задачи</SectionTitle>
        </motion.div>

        {tasks.slice(0, 5).map((task) => (
          <motion.div key={task.id} variants={stagger.item} className="px-5 pb-2">
            <TaskCard task={task} />
          </motion.div>
        ))}
            ...
            {habits.map(habit => (
              <div key={habit.id} onClick={() => toggleHabit(habit.id)}
                ...
                <div className="flex-1">
                  ...
                  {habit.streak > 0 && (
                    <div className="text-xs" style={{ color: 'var(--color-text3)' }}>🔥 {habit.streak} дней подряд</div>
                  )}
                </div>
                <span className="text-xs font-semibold" style={{ color: 'var(--color-gold)' }}>+{habit.xp} XP</span>
              </div>
            ))}
          </div>
        </motion.div>
